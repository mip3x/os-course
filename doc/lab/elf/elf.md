...
**Важно**: переменная `increment` символом не является! Также не являются символами `some_var` и `some_var_defined`. Работа с ними происходит через стек, поэтому изменяется лишь секция `.text`, линковщику не придётся совершать работу по разрешению этих символов.
