...
void bd_print() {
    for (int k = 0; k < nsizes; k++) {
        printf("size %d (blksz %ld nblk %d): free list: ", k, BLK_SIZE(k),
               NBLK(k));
        lst_print(&bd_sizes[k].free);
        printf("  alloc:");

        int n_block_pairs = NBLK(k) / 2;
        bd_print_vector(bd_sizes[k].xor_alloc, n_block_pairs);
...
void bd_mark(void *start, void *stop) {
    ...
    for (int k = 0; k < nsizes; k++) {
        ...
        for (; bi < bj; bi++) {
            ...
            pair_bit_invert(bd_sizes[k].xor_alloc, bi);
        }
    }
}
...
void bd_init(void *base, void *end) {
    ...
    // calling bd_print() after bd_init()
    bd_print();
}
